...
run.fn <- function(i){
    ...
    naive.cfo.res <- CFO.simu.fn(target, p.true, ncohort=ncohort, cohortsize=cohortsize,
                                init.level=1,  add.args=add.args)
...
file.name <- paste0("./PharStatR1/results/", "Simu_withCFO", nsimu, "_fix2_",  idx, ".RData")
